...
impl<'ast, T: Field> Flattener<'ast, T> {
            ...
            if *b {
                ...
                if i < len - 1 {
                    statements_flattened.push(FlatStatement::Definition(
                        size_unknown[i + 1],
                        FlatExpression::Mult(
                            Box::new(size_unknown[i].into()),
                            Box::new(is_not_smaller_run[i].into()),
                        ),
                    ));
                }
            } else {
                ...
                let or_left = FlatExpression::Sub(
                    Box::new(FlatExpression::Number(T::from(1))),
                    Box::new(size_unknown[i].clone().into()),
                );
                let or_right: FlatExpression<_> = FlatExpression::Sub(
                    Box::new(FlatExpression::Number(T::from(1))),
                    Box::new(a[i].clone().into()),
                );

                let and_name = self.use_sym();
                let and = FlatExpression::Mult(Box::new(or_left.clone()), Box::new(or_right.clone()));
                statements_flattened.push(FlatStatement::Definition(and_name, and));
                let or = FlatExpression::Sub(
                    Box::new(FlatExpression::Add(box or_left, box or_right)),
                    Box::new(and_name.into()),
                );
    ...
    fn flatten_if_else_expression<U: Flatten<'ast, T>>(
        ...
        statements_flattened.push(FlatStatement::Definition(
            term0_id,
            FlatExpression::Mult(
                Box::new(condition_id.clone().into()),
                Box::new(FlatExpression::from(consequence_id)),
            ),
        ));
        ...
        statements_flattened.push(FlatStatement::Definition(
            term1_id,
            FlatExpression::Mult(
                Box::new(FlatExpression::Sub(
                    box FlatExpression::Number(T::one()),
                    box condition_id.into(),
                    )),
                    Box::new(FlatExpression::from(alternative_id)),
            ),
        ));
        ...
        statements_flattened.push(FlatStatement::Definition(
            res,
            FlatExpression::Add(
                Box::new(FlatExpression::from(term0_id)),
                Box::new(FlatExpression::from(term1_id)),
            ),
        ));
    ...
    fn flatten_boolean_expression(
        ...
        match expression {
            ...
            BooleanExpression::Lt(box lhs, box rhs) => {
                ...
                // lhs
                {
                    ...
                    for i in 0..safe_width {
                        statements_flattened.push(FlatStatement::Condition(
                            ...
                            FlatExpression::Mult(
                                Box::new(FlatExpression::Identifier(lhs_bits_be[i])),
                                Box::new(FlatExpression::Identifier(lhs_bits_be[i])),
                            ),
                        ));
                    }
                ...
                // rhs
                {
                    ...
                    for i in 0..safe_width {
                        statements_flattened.push(FlatStatement::Condition(
                            ...
                            FlatExpression::Mult(
                                Box::new(FlatExpression::Identifier(rhs_bits_be[i])),
                                Box::new(FlatExpression::Identifier(rhs_bits_be[i])),
                            ),
                        ));
                    }
                ...
                for i in 0..bit_width {
                    statements_flattened.push(FlatStatement::Condition(
                        ...
                        FlatExpression::Mult(
                            Box::new(FlatExpression::Identifier(sub_bits_be[i])),
                            Box::new(FlatExpression::Identifier(sub_bits_be[i])),
                        ),
                    ));
                }
            ...
            BooleanExpression::BoolEq(box lhs, box rhs) => {
                ...
                let x_sub_y = FlatExpression::Sub(Box::new(x), Box::new(y));
                let name_x_mult_x = self.use_sym();

                statements_flattened.push(FlatStatement::Definition(
                    name_x_mult_x,
                    FlatExpression::Mult(Box::new(x_sub_y.clone()), Box::new(x_sub_y)),
                ));

                FlatExpression::Sub(
                    Box::new(FlatExpression::Number(T::one())),
                    Box::new(FlatExpression::Identifier(name_x_mult_x)),
                )
            }
            BooleanExpression::FieldEq(box lhs, box rhs) => {
                ...
                let x = self.flatten_field_expression(
                    symbols,
                    statements_flattened,
                    FieldElementExpression::Sub(Box::new(lhs), Box::new(rhs)),
                );
                ...
                statements_flattened.push(FlatStatement::Condition(
                    ...
                    FlatExpression::Mult(Box::new(x.clone()), Box::new(FlatExpression::Identifier(name_m))),
                ));

                let res = FlatExpression::Sub(
                    Box::new(FlatExpression::Number(T::one())),
                    Box::new(FlatExpression::Identifier(name_y)),
                );

                statements_flattened.push(FlatStatement::Condition(
                    FlatExpression::Number(T::zero()),
                    FlatExpression::Mult(Box::new(res.clone()), bBox::new(x)),
                ));
            ...
            BooleanExpression::UintEq(box lhs, box rhs) => {
                ...
                let x = FlatExpression::Sub(Box::new(lhs), Box::new(rhs));
                ...
                statements_flattened.push(FlatStatement::Condition(
                    FlatExpression::Number(T::zero()),
                    FlatExpression::Mult(Box::new(res.clone()), Box::new(x)),
                ));
            ...
            BooleanExpression::Le(box lhs, box rhs) => {
                let lt = self.flatten_boolean_expression(
                    symbols,
                    statements_flattened,
                    BooleanExpression::Lt(Box::new(lhs.clone()), Box::new(rhs.clone())),
                );
                let eq = self.flatten_boolean_expression(
                    symbols,
                    statements_flattened,
                    BooleanExpression::FieldEq(Box::new(lhs.clone()), Box::new(rhs.clone())),
                );
                FlatExpression::Add(Box::new(eq), Box::new(lt))
            }
            ...
            BooleanExpression::Or(box lhs, box rhs) => {
                ...
                statements_flattened.push(FlatStatement::Condition(
                    FlatExpression::Add(
                        Box::new(x.clone()),
                        Box::new(FlatExpression::Sub(box y.clone(), box name_x_or_y.clone().into())),
                    ),
                    FlatExpression::Mult(Box::new(x.clone()), Box::new(y.clone())),
                ));
                name_x_or_y.into()
            }
